...
#include "AutoNetworkConfigurator.h"
...
#endif
namespace inet {
Define_Module(AutoNetworkConfigurator);
#ifdef WITH_RADIO
using namespace inet::physicallayer;
#endif

/**
 * The AutoNetworkConfigurator model inherits from the Ipv4NetworkConfigurator model so that the
...
void AutoNetworkConfigurator::initialize(int stage)
{
 ...
 */
void AutoNetworkConfigurator::handleMessage(cMessage *msg)
{
    if (msg == timer) {
 ...
 */
void AutoNetworkConfigurator::reinvokeConfigurator(Topology& topology, cXMLElement *autorouteElement)
{
    ...
    topology.clear();
    AutoNetworkConfigurator::extractTopology(topology);
    AutoNetworkConfigurator::addStaticRoutes(topology, autorouteElement);
    AutoNetworkConfigurator::configureAllRoutingTables();
}

/**
 * Same implementation from the Ipv4NetworkConfigurator but it is needed to make sure that
 * the AutoNetworkConfigurator configureRoutingTable(Node *node) method is called as
 * this includes a unique implementation.
 */
void AutoNetworkConfigurator::configureAllRoutingTables()
{
    ensureConfigurationComputed(topology);
    ...
    for (int i = 0; i < topology.getNumNodes(); i++) {
        ...
        if (node->routingTable)
            AutoNetworkConfigurator::configureRoutingTable(node);
    }
}
 ...
 * this includes a unique implementation.
 */
void AutoNetworkConfigurator::configureRoutingTable(IIpv4RoutingTable *routingTable)
{
    ensureConfigurationComputed(topology);
 ...
 */
void AutoNetworkConfigurator::configureRoutingTable(Node *node)
{
