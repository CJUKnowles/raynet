...
#ifdef CLEANSLATE
#ifndef __CLEANSLATE_CC_H_
#define __CLEANSLATE_CC_H_
...
class CleanSlate : public TcpPacedNoCC, public RLInterface
{
public:
...
public: // General use
    ...
    bool takeActions = true; // Skips CleanSlate actions if false

    // CleanSlate parameters (Default values here, overridden in cleanSlate.ini)
    double rewardDelayForgiveness = 1; // Beta term from CleanSlate paper. Delay only degrades reward if RTT > baseRTT*rewardDelayForgiveness. Larger values emphasize aggressive throughputs by forgiving delay increases.
    double rewardLossMultiplier = 1;   // Zeta term from CleanSlate paper. Throughput is substracted by lossRate*rewardLossMultiplier in reward computation.  Larger values emphasize conservative throughputs by punishing loss. 
    ...
    double cleanSlateDelaySum=0.0;      // Used to hold the current sum of reported delays over a given interval. Used to compute an average at the end.
    ...
    double cleanSlateCwnd=0.0;          // The current congestion window (don't really need a new variable here, this is just useful for reference. Just use conn->snd_cwnd)
    double cleanSlateMaxThroughput=0.0; // The maximum delivery rate so far
    double cleanSlateMinDelay=9999;      // The minimum packet delay so far. Initialize to large value so the minimum is guaranteed to update.
    double cleanSlatePaceRate=1;        // Bytes sent per second. Usually smaller than cwnd.
    double cleanSlateDelayMetric=1;     // A measure of how close the currenty delay is to optimal. Will be 1 as long as the delay is within the forgiveness window.

    // CleanSlate helper variables (mostly used to facilitate computing the observations)
    ...
    bool first_slowstart_complete = false; // Do not take cleanSlate actions until the first slow start phase has completed. This allows the initial state (max througphut and min delay) to form naturally and prevents deadlocks.
