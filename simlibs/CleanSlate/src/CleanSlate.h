...
public: // General use
    ...
    // TcpPacedNoCC Overrides (These are mostly unchanged, and just used to gather statistic or disable automatic pacing)
    ...
    // CleanSlate configurable params
    double delayCoefficient; // Beta term from CleanSlate paper. Delay only degrades reward if RTT > baseRTT*delayCoefficient. Larger values emphasize aggressive throughputs by forgiving delay increases.
    double lossCoefficient;   // Zeta term from CleanSlate paper. Throughput is substracted by lossRate*lossCoefficient in reward computation.  Larger values emphasize conservative throughputs by punishing loss. 
