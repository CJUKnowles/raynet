...
def plot_experiment_summary(df, output_dir, output_name):
    plot_paths = [
        _plot_by_metric(df, metric="perf/ram_util_percent", output_path=output_dir+"Ram_util.pdf", title="RAM Utilization over time"),
        _plot_by_metric(df, metric="perf/cpu_util_percent", output_path=output_dir+"CPU_util.pdf", title="CPU Utilization over time"),
        _plot_by_metric(df, metric="perf/gpu_util_percent0", output_path=output_dir+"GPU_util.pdf", title="GPU Utilization over time")
    ]
    
    for metric in df.columns:
        if "return" in metric or "reward" in metric:
